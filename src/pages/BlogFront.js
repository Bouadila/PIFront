...
export default function BlogFront() {
  ...
  const getMultipleFiles = async () => {
    try {
      const { data } = await axios.get('https://cors-anywhere.herokuapp.com/https://lernigoback.herokuapp.com/eya/getMultipleFiles');
      return data;
    } catch (error) {
      throw error;
    }
  ...
  useEffect(() => {
    ...
    socket.current.on('new-notification', () => {
      ...
      Swal.fire('the feed is up to date!', 'You clicked the button!', 'success');
    ...
    (function (d, m) {
      var kommunicateSettings = {
        appId: 'da13654b0c21d8a6200129043a9f84f5',
        popupWidget: true,
